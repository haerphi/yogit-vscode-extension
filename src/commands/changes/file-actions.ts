...
export function registerFileActions(): vscode.Disposable[] {
    const openFile = vscode.commands.registerCommand('haerphi-yorgit.open-file', async (node: ChangeLeaf) => {
        await vscode.commands.executeCommand('vscode.open', node.change.uri);
    });

    const copyFileName = vscode.commands.registerCommand('haerphi-yorgit.copy-file-name', async (node: ChangeLeaf) => {
