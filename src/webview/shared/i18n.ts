 ...
 * injecte la langue de VS Code via `window.__YORGIT_LOCALE__` (script inline
...
export function isFrench(): boolean {
    const w = window as typeof window & { __YORGIT_LOCALE__?: string };
    const lang = (w.__YORGIT_LOCALE__ ?? navigator.language ?? '').toLowerCase();
